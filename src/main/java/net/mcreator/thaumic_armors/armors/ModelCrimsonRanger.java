...
import net.mcreator.thaumic_armors.armors.ItemCrimsonRanger;
...
/**
 * Cultist_ranger.tcn - TechneToTabulaImporter Created using Tabula 4.1.1
 */
public class ModelCrimsonRanger extends ModelBiped {
	...
	public ModelCrimsonRanger(float f) {
		 ...
		 * this.Mask.addBox(-4.5F, -5.0F, -4.599999904632568F, 9, 5, 1,
		 * 0.0F);
		 */
	...
	private void checkSet(Entity entity) {
		if (entity instanceof EntityLivingBase && entity.ticksExisted % 20 == 0) {
			...
			for (int a = 1; a < 4; ++a) {
				...
				if (piece != null && piece.getItem() instanceof ItemCrimsonRanger) {
					++set;
					if (a == 3) {
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
		...
		int mask = hasMask.containsKey(Integer.valueOf(entity.getEntityId())) ? ((Integer) hasMask.get(Integer.valueOf(entity.getEntityId())))
				.intValue() : -1;
		this.Mask[0].isHidden = true;
		this.Mask[1].isHidden = false;
		/*
		 * for(int var11 = 0; var11 < 2; ++var11) { if(mask == var11) {
		 * this.Mask[var11].isHidden = false; } else {
	...
	public void setRotationAnglesZombie(float p_78087_1_, float p_78087_2_, float p_78087_3_, float p_78087_4_, float p_78087_5_,
